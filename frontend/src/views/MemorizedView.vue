...
const mode = ref(localStorage.getItem('memorizedMode') || 'list')
watch(mode, (val) => localStorage.setItem('memorizedMode', val))
...
async function loadMore() {
  ...
  loading.value = true
  try {
    const { data } = await request.get('/users/me/memorized/paged', {
...
async function handleUnmarkMemorized(wordId) {
  try {
    await request.delete(`/users/me/memorized/${wordId}`)
    message.success(t('unmarkedMemorized'))
    // 从列表移除
  ...
  <div class="collection-container">
    <n-card>
      ...
      <template v-if="mode === 'card'">
        <div v-if="currentCard()">
          <flash-card
            ...
            @toggle-memorized="handleUnmarkMemorized"
          />
        </div>
        <n-empty v-else-if="!cardLoading" :description="t('noMemorized')">
          <template #extra>
      ...
      <template v-else>
        ...
        <n-empty v-else-if="!loading" :description="t('noMemorized')" />
