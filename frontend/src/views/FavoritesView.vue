...
const mode = ref(localStorage.getItem('favoritesMode') || 'list')
watch(mode, (val) => localStorage.setItem('favoritesMode', val))
...
async function loadMore() {
  ...
  loading.value = true
  try {
    const { data } = await request.get('/users/me/favorites/paged', {
  ...
  <div class="collection-container">
    <n-card>
      ...
      <template v-if="mode === 'card'">
        ...
        <n-empty v-else-if="!cardLoading" :description="t('noFavorites')">
          <template #extra>
      ...
      <template v-else>
        ...
        <n-empty v-else-if="!loading" :description="t('noFavorites')" />
